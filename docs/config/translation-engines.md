...
如果服务商提供 OpenAI Chat Completions 兼容接口，可以通过“+”添加“自定义接口”供应商，并填写接口地址、API Key 和模型 ID。还可以为该实例单独设置代理、system/user 模板和附加请求体；这些设置不会改变其他服务实例的模型或凭据。填写完成后使用右上角“检查连接”发送一条最小请求确认配置。
