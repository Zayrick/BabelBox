...
3. 添加 AI 服务时，点击搜索栏右侧的“+”，选择供应商，再依次填写服务名称、模型 ID、请求信息和凭据。名称可以自定义，默认格式为“模型ID - 服务商”，图标沿用所选供应商的图标；自定义请求体位于默认折叠的“高级请求参数”中，可按多行 JSON 编辑。
4. 保存后可以点击“检查连接”发送一条很短的真实请求；这可能产生少量用量。
5. 确认结果、耗时和额度都符合预期，再处理长页面。
...
如果服务商提供 OpenAI Chat Completions 兼容接口，可以通过“+”添加“自定义接口”供应商，并填写接口地址、模型 ID，以及服务端需要时使用的 API Key。API Key 留空时不会发送鉴权信息。还可以为该实例单独设置代理、system/user 模板和附加请求体；这些设置不会改变其他服务实例的模型或凭据。填写完成后使用右上角“检查连接”发送一条最小请求确认配置。

模型输入会提供适合该供应商的候选项，也允许直接填写供应商支持的模型 ID。若想保留同一供应商的多个模型，请分别添加为多个服务实例，而不是反复覆盖一个全局模型选择。
