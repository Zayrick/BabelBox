...
打开 BabelBox 弹窗，使用“恢复原文”。BabelBox 会移除双语模式插入的译文，并把仅译文模式处理过的文本恢复为原文。
...
只填写在 BabelBox 设置页的服务配置中，不要放进网页、截图、Issue、聊天记录或 Git 仓库。若怀疑已经泄露，请立即在服务商控制台撤销并重新生成。

默认的“本机保险库”会加密保存凭据，浏览器重启或扩展正常更新后无需重新填写。共享设备可在“配置管理 → API 凭据存储”切换为“仅本次会话”。
...
在 [GitHub Issues](https://github.com/Zayrick/BabelBox/issues) 中提供：
