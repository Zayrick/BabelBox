...
vi.mock("@/src/core/translation/public", () => {
    const durableProtectedSelector = [
        ...
        ".katex", ".notranslate", "[translate='no']", "[data-notranslate='true']",
    ].join(",");
    ...
    const textSlots = (
        ...
        while (current) {
            ...
            if (source.trim() && node.parentElement && !shouldStayOriginal(node.parentElement) &&
    ...
    return {
        ...
        ) => textSlots(element, shouldStayOriginal).map(({source}) => source).join(""),
...
describe("全文翻译可见性锚点", () => {
    ...
    it("临时隐藏保留译文，隐藏期间的原文更新仍会使译文失效", async () => {
        ...
        runtime.candidates = [{element: message, kind: "content", reason: "message"}];
